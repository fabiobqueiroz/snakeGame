...
const conterTime = document.querySelector(".counter");
...
function update() {
    if (beginGame) {
        if (gameOver()) {
            ...
            conterTime.style.display = "block";
            setInterval(() => {
                conterTime.innerHTML -= 1;
                if (conterTime.innerHTML == "0") location.reload();
            }, 1000);
            return;
        };
        setTimeout(() => {
...
function gameOver() {
    ...
    const hitBottonWall = snake[0].y > fild.height - 10;
    return hitLeftWall || hitRightWall || hitTopWall || hitBottonWall
};
function generarFood() {
    food_x = Math.floor((Math.random() * ((fild.width - 10) - 0) + 10) / 10) * 10;
    food_y = Math.floor((Math.random() * ((fild.height - 10) - 0) + 10) / 10) * 10;
    snake.forEach((part) => {
